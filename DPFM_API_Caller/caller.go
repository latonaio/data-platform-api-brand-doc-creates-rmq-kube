...
import (
	"context"
	"crypto/sha256"
	dpfm_api_input_reader "data-platform-api-brand-doc-creates-rmq-kube/DPFM_API_Input_Formatter"
	dpfm_api_output_formatter "data-platform-api-brand-doc-creates-rmq-kube/DPFM_API_Output_Formatter"
	"data-platform-api-brand-doc-creates-rmq-kube/config"
	"encoding/base64"
	"encoding/hex"
	"fmt"
...
func (c *DPFMAPICaller) AsyncDocCreates(
	...
	var fileExtension = input.Brand.HeaderDoc.FileExtension
	var docData = input.DocData

	var combinedString = fmt.Sprintf(
		"%v%v",
		time.Now().Unix(),
		input.Brand.HeaderDoc.DocVersionID,
	)
	...
	response := c.createSqlProcess(input, output, &dpfm_api_output_formatter.HeaderDoc{
		...
		DocType:                  input.Brand.HeaderDoc.DocType,
		DocVersionID:             input.Brand.HeaderDoc.DocVersionID,
		DocID:                    &dockId,
		FileExtension:            input.Brand.HeaderDoc.FileExtension,
		FileName:                 input.Brand.HeaderDoc.FileName,
		FilePath:                 directoryPath,
		DocIssuerBusinessPartner: input.Brand.HeaderDoc.DocIssuerBusinessPartner,
	}, errs, log)
